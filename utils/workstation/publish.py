...
from utils.multichain import compute_inf
...
def export_shareable_html_bytes(catalog: Catalog, job_id: str) -> Tuple[bytes, str]:
    ...
    metrics = _recompute_metrics(viewer, baked_fr3d, overrides)
    ...
    with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        for path in sorted(viewer.rglob("*")):
            ...
            if rel in baked_fr3d:
                ...
                zf.writestr(rel, _rewrite_inf_bar(html, metrics.get("inf")))
...
def _recompute_metrics(
    ...
    """Update metrics.json INF from baked annotations when both panels exist."""
...
def _rewrite_inf_bar(html: str, inf: Optional[Dict[str, Any]]) -> str:
    """Replace the compare INF bar in index.html with recomputed values."""
    if not inf:
        return html
    new_bar = _format_inf_metrics(inf)
    if not new_bar:
        return html
    updated, n = re.subn(
