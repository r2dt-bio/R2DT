...
def extract_sequence_from_cif(
    ...
    try:
        ...
        # Drop crystallographic symmetry mates. FR3D emits copies generated by
        # non-identity symmetry operators (unit ids like '…|G|1||||7_555'); keeping
...
def _is_symmetry_mate(unit_id: str) -> bool:
    ...
    operator. Asymmetric-unit residues have it empty or set to the identity
    '1_555'; symmetry mates carry e.g. '7_555'.
    """
    parts = unit_id.split("|")
    if len(parts) < 9:
        return False
    operator = parts[8].strip()
    return bool(operator) and operator != "1_555"
